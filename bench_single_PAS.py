...
# def update_data_single_thread(item_list, scenario):
#     psicontainer[scenario].save('FY24_9F_TEST-FINANCIAL_single_before.elc')
#     for item in item_list:
#         print('Starting: ' + item['PRODUCT'])
#         def pr(new_value):
#             item["PROGRESS"] = [new_value/100]  # スパークライン用のデータを更新
#             item["pct"] = new_value/100  # 数値を表示する列を更新
#         item['pct'] = 0                                                                         # set progress-modal 'pct' column to zero
#         item['PROGRESS'] = 0      
#         psicontainer[scenario].batchCalculatePSI(model_code=item['PRODUCT'],callback=pr)
#         print(f"{item['PRODUCT']} is finished!")
#     psicontainer[scenario].save('FY24_9F_TEST-FINANCIAL_single_after.elc')
...
#     update_data_single_thread(modal_data, scenario_name)
...
#     print(f"update_data_single_thread completed in {elapsed_time:.2f} seconds.")
...
def update_data_single_thread(item_list, scenario):
    psicontainer[scenario].save('FY24_9F_TEST-FINANCIAL_single_PAS_before.elc')
    for item in item_list:
        print('Starting: ' + item['PRODUCT'])
        def pr(new_value):
            item["PROGRESS"] = [new_value/100]  # スパークライン用のデータを更新
        ...
        psicontainer[scenario].batchCalculatePSI(model_code=item['PRODUCT'],callback=pr)
        print(f"{item['PRODUCT']} is finished!")
    psicontainer[scenario].save('FY24_9F_TEST-FINANCIAL_single_PAS_after.elc')
